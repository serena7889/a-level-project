...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      $htmlString .= '<tr class="clickable-row" data-test="test data" data-convo="' . $row['conversationID'] . '"><td><b>' . $row['jobTitle'] . '</b><br>' . $row['studentFirstName'] . ' ' . $row['studentLastName'] . '</td>';
    }
    echo $htmlString;
} else {
    echo "0 results";
